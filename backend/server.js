...
io.on("connection", (socket) => {
    ...
    // a user joins room anyway when a erquest is accepted a chat is made then 
        ...
        try {
            ...
            chat.users.forEach((user) => {
                if (user.toString() !== sender) {
                    io.to(user.toString()).emit("receive-message", {
                        sender,
                        chatId,
                        content,
    ...
    socket.on("Not-typing", async({ sender, chatId }) => {
        // console.log("nooo")
          ...
          chat.users.forEach(async(user)=>{
              let userId = user.toString();
            if(userId !== sender){
                io.to(userId).emit("Not-typing",{
                    userId, chatId, sender
                })
            }
          })
    })
    ...
    socket.on("message-read" ,async({chatId,sender,messageId})=>{
